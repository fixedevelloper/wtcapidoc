...
class StaticController extends Controller
{
    public function dashboard(Request $request)
    {
        ...
        $last_transactions=Transaction::query()->where(['type'=>Helper::TYPESANDBOX])
        ...
        $sumWeekendTransactions = Transaction::query()->where('customer_id',$customer->id)
            ->where('status',Helper::STATUSSUCCESS)
            ->where('type',Helper::TYPESANDBOX)->whereBetween('created_at', [$startOfWeekend, $endOfWeekend])
            ->sum('amount');
        $sumTotal=Transaction::query()->where('customer_id',$customer->id)
            ->where('status',Helper::STATUSSUCCESS)
            ->where('type',Helper::TYPESANDBOX)->sum('amount');
        $startOfMonth = Carbon::now()->startOfMonth();
        $endOfMonth = Carbon::now()->endOfMonth();
        $sumCurrentMonthTransactions = Transaction::query()->where('customer_id',$customer->id)
            ->where('status',Helper::STATUSSUCCESS)
            ->where('type',Helper::TYPESANDBOX)->whereBetween('created_at', [$startOfMonth, $endOfMonth])
            ->sum('amount');
        $sumDepositTotal=DepositRequest::query()
            ->where('status',Helper::STATUSSUCCESS)
